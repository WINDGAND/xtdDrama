...
\begin{center}
  ...
  \begin{tcolorbox}[dramabox]
    ...
    建议体验路径：登录 $\to$ 上传一张日常图片 $\to$ 点击感兴趣的 Drama 风格方向 $\to$ 等待约 10\,s 生成 $\to$ 发布至广场，NPC 将在数秒内互动
  \end{tcolorbox}
  \end{minipage}
...
\noindent\begin{tabular}{@{}p{2.2cm}
  ...
  \kw{生成示例} & 屏幕边缘浮现像素 ERROR 警告与火焰特效、背景闪现电流光效、手边咖啡杯变身卡通愤怒表情 \\[3pt]
  \kw{核心价值} & 保留人物与桌面的物理结构，用视觉特效把"内心崩溃"外化成有意思的梗图，把苦逼变成好玩 \\
\end{tabular}
...
\begin{tikzpicture}[node distance = 0.5cm]
  ...
  \node[pipe, draw=AppleBlue!70,   fill=LightBlue,  right=0.5cm of p1] (p2)
    {\textbf{决策层}\\{\scriptsize\model{hunyuan}}};
...
\begin{description}[leftmargin=0em, style=nextline, itemsep=0.8em, topsep=3pt]
  ...
  \item[\kw{Guess \& Refine 情绪互动罗盘（决策层）}]\mbox{}\\
    告别冷冰冰的提示词输入框。语言模型（\model{hunyuan}）根据感知结果自动生成一句共情破冰文案，并同步给出 3 个风格各异的 Drama 改造方向供用户一键选择，完成意图收敛。支持"换一批"，每次重新生成语义不重复的新选项。
...
图~\ref{fig:arch} 展示产品的三层能力架构：用户端负责交互与上传，AI 能力核心完成感知–决策–生成–反馈全链路，技术底座提供模型与数据支撑。
...
\begin{tikzpicture}[node distance = 0.5cm and 2.2cm]
  ...
  \node[archTH, below=of t1]       (t2) {语言与决策模型\\hunyuan};
  ...
  \begin{scope}[on background layer]
    \node[grpbox={TokenPurple},
          fit=(t1)(t2)(t3),
          label={[font=\footnotesize\bfseries,color=TokenPurple]above:腾讯 TokenHub MaaS}]
          (gTH) {};
...
\caption{感知--决策--生成链路（赛题第 3、4 问直接回应）}
\label{fig:flow1}
\end{figure}
...
小题大Drama 深度融合三层 AI 能力，缺任意一层产品链路即断裂：
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{>{\bfseries}p{1.4cm} p{3.5cm} X}
    ...
    \vspace{4pt}决策层
      & \tmod{hunyuan}\newline（TokenHub）
      & 产品的「大脑和嘴巴」：输出一句击中情绪的俏皮破冰文案 + 3 个各不相同的 Drama 方向（内嵌结构锚定指令）；同时驱动 5 位 NPC 角色生成个性化评论，承担情绪校准与场景描述补全等辅助推理任务。 \\
    \addlinespace[3pt]
    \vspace{4pt}生成层
...
\begin{tcolorbox}[dramabox]
  \kw{结论：去掉 AI，本产品将彻底崩溃——三层 AI 缺一不可，且相互依赖，无法用传统技术等价替代。}
\end{tcolorbox}
...
\subsubsection*{感知层：从图片到结构化语义（直接回应赛题第 4 问）}

\begin{itemize}[leftmargin=2em, itemsep=0.3em, topsep=3pt]
  \item \kw{精准感知，从不"看不懂"}：\model{youtu-vita} 多模态模型逐帧解读图片，输出「主实体 + 场景 + 情绪」的结构化理解。对任意构图、光线、风格的日常照片，均能稳定产出有意义的分析结果。
  \item \kw{多重容错，零死胡同}：AI 输出质量波动时，系统自动在多条解析路径之间自适应切换，必要时启动双模型协同重组——用户看到的永远是完整的改造建议，而非"分析失败，请重试"。
  \item \kw{情绪智能校准，「猜对你的心情」}：感知层内置情绪置信度评估；当首次判断不确定时，自动发起第二轮情绪复核，确保 AI 的破冰文案击中用户当下的真实情绪，而非随机标注。
  \item \kw{图片类型差异化}：按人像/食物/场景/宠物/物品等 6 类图片类型，智能匹配最适合的 Drama 改造方向——食物自动偏美食插画感，宠物自动偏 Q 版萌化，做到「每张图的改造建议都是量身定制」。
\end{itemize}
...
\begin{itemize}[leftmargin=2em, itemsep=0.3em, topsep=3pt]
  ...
  \item \kw{三条 Drama 铁律保证生成质量}：每个选项的生图指令内嵌三层约束——物理锚定（原图骨架不变）、轻度逻辑错位（尺度/材质/精灵具现三类手法择一）、虚实交融克制感（Drama 元素「藏在真实里」而非喧宾夺主）；用户选哪个方向，生成结果都不会"崩"。
  \item \kw{换一批，真的不重复}：「换一批」功能在规则层和语义层双重保障方向差异化，用户无论点多少次，每批 3 个选项都是全新的改造思路，防止「AI 总在循环」的体验疲倦。
\end{itemize}

\subsubsection*{生成层：风格与结构一致性保障（直接回应赛题第 2 问）}
...
\begin{enumerate}[leftmargin=2em, itemsep=0.3em, topsep=3pt, label=\textbf{\arabic*.}]
  ...
  \item \kw{语义锚定指令（文本层）}：每条生图 Prompt 首句强制声明「请严格锁定原图的〔物理元素清单〕，物理骨架完全不变」，在文字层面约束模型不改变实体身份与空间关系，与参考图约束形成双重保障。
...
\begin{itemize}[leftmargin=2em, itemsep=0.25em, topsep=3pt]
  ...
  \item \kw{TokenHub}：统一 Bearer API Key 接入四种能力；封装层（\code{src/lib/tokenhub.ts}）包含 429/5xx 指数退避重试（500\,ms 延迟，最多 1 次）与 30\,s 超时控制。
\end{itemize}
...
\begin{itemize}[leftmargin=2em, itemsep=0.25em, topsep=3pt]
  ...
  \item \kw{感知两阶段兜底}：\model{youtu-vita} 格式异常时，\model{hunyuan} 自动二次结构化重组，感知层整体成功率显著提升，避免因模型输出格式漂移导致链路中断。
  \item \kw{镜像落盘}：生成结果 URL 在发布时同步拉取并持久化至 Supabase Storage，广场内容不依赖模型临时 CDN 链接；若拉取失败则拒绝发布并给出明确错误提示。
\end{itemize}

\subsection{商业化思考}

相较于纯技术 Demo，小题大Drama 具备强 C 端变现与长期运营潜力：

\vspace{3pt}
\begin{description}[leftmargin=0em, style=nextline, itemsep=0.5em, topsep=3pt]
  \item[\kw{基础商业模式（增值服务）}]
    每日 3 次免费基础图片生成；更高分辨率、视频/Live 图生成、无水印下载等，通过"小题大Drama VIP"或绑定 QQ 超级会员（SVIP）权益解锁。

  \item[\kw{情绪价值变现（付费 NPC 陪伴）}]
    基础 5 位 NPC 免费；付费解锁专属 IP 角色陪聊（如甄嬛传人设、王者英雄等），获得更强拟真互动体验。

  \item[\kw{ToB 品牌营销（定制风格与流量裂变）}]
    结合开学季、游戏新赛季等节点，与品牌联名推出定制风格库；品牌承担算力成本，获得 UGC 裂变曝光。
\end{description}

\subsubsection*{接入腾讯 PCG 业务的可行性}

\begin{itemize}[leftmargin=2em, itemsep=0.25em, topsep=3pt]
  \item \kw{QQ 空间}：作为"夸张版动态发布插件"，激活空间活跃度，与现有相册/动态场景无缝衔接
  \item \kw{腾讯视频}：截图名场面一键生成发疯梗图，参与弹幕/评论互动生态
  \item \kw{QQ 群聊/频道}：作为群内 Bot，接收成员发图，丰富斗图文化
\end{itemize}
...
\begin{quote}
"优秀的 AI 技术不应只在实验室里跑分，更应该走进年轻人的日常，化解他们生活中的焦虑与无聊。"
\end{quote}

\noindent 小题大Drama 通过重构用户的视觉表达与社交反馈链条，切中当代年轻群体的情绪命脉：既是对赛题"AI 玩转视觉互动"的标准答卷，也是腾讯 PCG 在 AI 原生社交娱乐赛道上一条具备商业潜力的创新探索路径。
