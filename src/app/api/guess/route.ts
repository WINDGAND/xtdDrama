...
/* ----------------------------------------------------------------
 * 三轴轴词缀（规则去重用）
 * 轴重命名：插画→史诗剧照 / 胶片→手绘日记 / 设计→胶片年代
 * ---------------------------------------------------------------- */
const AXIS_KEYWORDS = {
  illustration: ["dramatic chiaroscuro lighting", "cinematic wide-angle reframe", "high-contrast natural light", "strong directional shadows", "theatrical yet realistic"],
  film: ["expressive hand-drawn illustration", "loose emotive linework", "warm ink and wash", "editorial diary sketch", "emotionally charged brushwork"],
  design: ["warm vintage analog film", "nostalgic color cast", "35mm film grain texture", "soft natural vignette", "candid documentary warmth"],
} as const;
...
每次必须严格按以下三轴生成，每轴对应一个 option，禁止跨轴混搭。**三轴的核心差异在于"用什么手法放大情绪反差"**：

**id=1 史诗剧照轴**
- title 参考：史诗剧照感、电影定格感、光影戏剧感、大片重构感
- prompt 必须包含以下词缀之一：dramatic chiaroscuro lighting / cinematic wide-angle reframe / high-contrast natural light / strong directional shadows / theatrical yet realistic
- 风格方向：将平凡瞬间重构为电影级别的定格画面——戏剧性自然光、强方向性阴影、宽画幅构图重组，像一张电影剧照，主体仍清晰可辨但气场截然不同
- **情绪放大方式**：通过光影对比与构图张力放大情绪，让观者感受到"这一刻比想象中更重"

**id=2 手绘日记轴**
- title 参考：手绘日记感、插画日记风、情绪草稿感、手稿速写感
- prompt 必须包含以下词缀之一：expressive hand-drawn illustration / loose emotive linework / warm ink and wash / editorial diary sketch / emotionally charged brushwork
- 风格方向：用情绪化的手绘笔触将场景转化为"有温度的日记插图"——不追求精准，追求情绪真实；松动线条、墨水晕染，像画者当下心情的直接投射
- **情绪放大方式**：用笔触的松弛与紧张感表达情绪，让观者感受到"是一个真实的人画下了这个瞬间"

**id=3 胶片年代轴**
- title 参考：胶片年代感、温柔纪实感、复古影像感、模拟胶片感
- prompt 必须包含以下词缀之一：warm vintage analog film / nostalgic color cast / 35mm film grain texture / soft natural vignette / candid documentary warmth
- 风格方向：用35mm胶片的质感与色调将场景拉回到"那个年代"——温暖色偏、自然颗粒感、柔和暗角；不是滤镜，是把今天的瞬间放进了另一个时代的温度里
- **情绪放大方式**：通过时间感与怀旧温度放大情绪，让观者感受到"这个瞬间值得被永久保存"
...
- **说明情绪变化方向**：告诉用户这个风格会把画面"情绪改造成什么"，而不是重复 title 的意思
- 三个 description 之间必须有明显差异，突出每条独有的"情绪放大手法"
- 示例思路：「用戏剧光影让[主体]像电影剧照」「把[场景]画成有温度的手绘日记」「用胶片颗粒给[瞬间]一点时间重量」

## SDXL prompt 结构化公式

每个 option 的 prompt 必须按以下顺序组装（40-80词）：
1. **主体不变锚定**（必填，每条 prompt 开头）：\`preserve original subject and composition, keep subject clearly recognizable,\`
2. **轴专属美学词缀**（必填，对应上方三轴词缀中至少一个）
3. **情绪氛围改写**（必填，根据 mainEntity + sceneState + userEmotion + imageType 描述戏剧化改写方向，1-2 句，必须体现情绪放大而非仅视觉美化）
4. **质量词缀**（必填，结尾固定）：\`high quality, detailed, 4k\`

禁止词缀（不得出现）：neon glow, neon accents, cyberpunk aesthetic, dark gothic atmosphere, horror elements, blood, violence, grotesque, eldritch, nsfw
...
- **三轴的情绪放大手法必须不同**：光影戏剧感 vs 笔触情绪感 vs 时间温度感
...
/** 提取 prompt 里命中了哪个轴（史诗剧照/手绘日记/胶片年代） */
function detectAxis(prompt: string): "illustration" | "film" | "design" | null {
  const p = prompt.toLowerCase();
  if (AXIS_KEYWORDS.illustration.some((k) => p.includes(k.toLowerCase()))) return "illustration";
  if (AXIS_KEYWORDS.film.some((k) => p.includes(k.toLowerCase()))) return "film";
  if (AXIS_KEYWORDS.design.some((k) => p.includes(k.toLowerCase()))) return "design";
  return null;
}
